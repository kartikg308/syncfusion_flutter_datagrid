...
class SfDataPagerState extends State<SfDataPager> {
      ...
      return Icon(iconData,
          ...
          color: visible
              ? _dataPagerThemeHelper!.brightness == Brightness.light
                  ? _dataPagerThemeHelper!.disabledItemTextStyle.color?.withOpacity(0.54)
                  : _dataPagerThemeHelper!.disabledItemTextStyle.color?.withOpacity(0.65)
  ...
  double _getDataPagerWidth({bool canEnableDataPagerLabel = false, bool isDropDown = false}) {
    if (widget.direction == Axis.horizontal) {
    ...
    if (pagerItem == null) {
      if (element == null) {
        ...
        pagerItem = Text(
          index.toString(),
          key: element.key,
          style: isSelected ? _dataPagerThemeHelper!.selectedItemTextStyle : _dataPagerThemeHelper!.itemTextStyle,
        );
      ...
      child: Padding(
        ...
        child: CustomPaint(
          ...
          child: Material(
            ...
            child: InkWell(
              key: pagerItemKey,
              mouseCursor: visible ? MaterialStateMouseCursor.clickable : SystemMouseCursors.basic,
              splashColor: visible ? flutterTheme.splashColor : Colors.transparent,
              hoverColor: visible ? flutterTheme.hoverColor : Colors.transparent,
              highlightColor: visible ? flutterTheme.highlightColor : Colors.transparent,
              onTap: () {
                if (element != null) {
    ...
    if (widget.onRowsPerPageChanged != null) {
      ...
      return Padding(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(3.0), border: Border.all(color: _dataPagerThemeHelper!.dropdownButtonBorderColor)),
  ...
  void _buildDataPagerWithLabel(BoxConstraints constraint, List<Widget> children) {
    ...
    // DataPager
    final BoxConstraints dataPagerConstraint = BoxConstraints(maxWidth: canEnablePagerLabel ? _getTotalDataPagerWidth(constraint) - getRowsPerPageLabelWidth() : _getTotalDataPagerWidth(constraint), maxHeight: _getTotalDataPagerHeight(constraint));
  ...
  void didChangeDependencies() {
    ...
    _dataPagerThemeHelper = DataPagerThemeHelper(_dataPagerThemeData, Theme.of(context).colorScheme, themeData);
      ...
      child: LayoutBuilder(builder: (BuildContext context, BoxConstraints constraint) {
        ...
        if (_isDesktop && widget.direction == Axis.horizontal) {
          ...
          if (widget.onRowsPerPageChanged != null && widget.direction == Axis.horizontal) {
            ...
            return SizedBox(width: _getDataPagerWidth(), height: _getDataPagerHeight(), child: dataPager);
          }
        }
      }),
    );
...
class DataPagerThemeHelper {
  ...
  DataPagerThemeHelper(SfDataPagerThemeData? sfDataPagerThemeData, ColorScheme? colorScheme, ThemeData themeData) {
    ...
    selectedItemTextStyle = sfDataPagerThemeData.selectedItemTextStyle ?? TextStyle(fontFamily: 'Roboto', fontWeight: FontWeight.w400, fontSize: 14, color: colorScheme!.onPrimary);
    disabledItemColor = sfDataPagerThemeData.disabledItemColor ?? Colors.transparent;
    disabledItemTextStyle = sfDataPagerThemeData.disabledItemTextStyle ?? TextStyle(fontFamily: 'Roboto', fontWeight: FontWeight.w400, fontSize: 14, color: colorScheme!.onSurface.withOpacity(0.36));
    itemBorderColor = sfDataPagerThemeData.itemBorderColor ?? Colors.transparent;
    itemBorderWidth = sfDataPagerThemeData.itemBorderWidth;
    itemBorderRadius = sfDataPagerThemeData.itemBorderRadius ?? BorderRadius.circular(50);
    dropdownButtonBorderColor = sfDataPagerThemeData.dropdownButtonBorderColor ?? colorScheme!.onSurface.withOpacity(0.12);
  }
