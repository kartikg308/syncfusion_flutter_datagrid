...
class SfDataGrid extends StatefulWidget {
  ...
  const SfDataGrid({
    ...
    @Deprecated('use SfDataGrid.showColumnHeaderIconOnHover instead') this.showFilterIconOnHover = false,
...
class SfDataGridState extends State<SfDataGrid> with SingleTickerProviderStateMixin {
  ...
  void _onDataGridThemeDataChanged(SfDataGridThemeData? newThemeData, ColorScheme? newColorScheme) {
    ...
    if (canRefreshView) {
      _dataGridThemeHelper = DataGridThemeHelper(_dataGridThemeData, _dataGridConfiguration.colorScheme);
  ...
  void _updateSummaryColumns(int columnIndex) {
    final String columnName = _dataGridConfiguration.columns[columnIndex].columnName;
  ...
  Future<void> _processUpdateDataSource() async {
    ...
    if (widget.selectionMode != SelectionMode.none) selection_manager.removeUnWantedDataGridRows(_dataGridConfiguration);
  ...
  Future<void> _updateProperties(SfDataGrid oldWidget) async {
    ...
    final bool isColumnsCollectionChanged = _columns!.length != widget.columns.length;
    ...
    final bool canApplyFiltering = isColumnsChanged && _columns!.length != widget.columns.length;
  ...
  void _updateBoxPainter() {
    if (widget.selectionMode == SelectionMode.multiple && widget.navigationMode == GridNavigationMode.row) {
  ...
  void moveCurrentCellTo(RowColumnIndex rowColumnIndex) {
    if (_dataGridStateDetails != null) {
      ...
      if (rowColumnIndex != RowColumnIndex(-1, -1) && dataGridConfiguration.selectionMode != SelectionMode.none && dataGridConfiguration.navigationMode != GridNavigationMode.row) {
        ...
        final int columnIndex = grid_helper.resolveToGridVisibleColumnIndex(dataGridConfiguration, rowColumnIndex.columnIndex);
...
class DataGridThemeHelper {
  ...
  DataGridThemeHelper(SfDataGridThemeData? dataGridThemeData, ColorScheme? colorScheme) {
    ...
    selectionColor = dataGridThemeData.selectionColor ?? colorScheme!.onSurface.withOpacity(0.08);
    headerHoverColor = dataGridThemeData.headerHoverColor ?? colorScheme!.onSurface.withOpacity(0.04);
    rowHoverColor = dataGridThemeData.rowHoverColor ?? colorScheme!.onSurface.withOpacity(0.04);
    sortIconColor = dataGridThemeData.sortIconColor ?? colorScheme!.onSurface.withOpacity(0.6);
    frozenPaneLineColor = dataGridThemeData.frozenPaneLineColor ?? colorScheme!.onSurface.withOpacity(0.38);
    columnResizeIndicatorColor = dataGridThemeData.columnResizeIndicatorColor ?? colorScheme!.primary;
    columnResizeIndicatorStrokeWidth = dataGridThemeData.columnResizeIndicatorStrokeWidth ?? 2;
    currentCellStyle = dataGridThemeData.currentCellStyle ?? DataGridCurrentCellStyle(borderColor: colorScheme!.onSurface.withOpacity(0.26), borderWidth: 1.0);
    rowHoverTextStyle = dataGridThemeData.rowHoverTextStyle ?? TextStyle(fontFamily: 'Roboto', fontWeight: FontWeight.w400, fontSize: 14, color: colorScheme!.onSurface.withOpacity(0.87));
    ...
    sortOrderNumberBackgroundColor = dataGridThemeData.sortOrderNumberBackgroundColor;
    _filterPopupTextStyle = TextStyle(fontSize: 14.0, color: colorScheme!.onSurface.withOpacity(0.89), fontFamily: 'Roboto', fontWeight: FontWeight.normal);

    _filterPopupDisabledTextStyle = TextStyle(fontSize: 14.0, color: colorScheme.onSurface.withOpacity(0.38), fontFamily: 'Roboto', fontWeight: FontWeight.normal);

    filterPopupTextStyle = dataGridThemeData.filterPopupTextStyle != null ? _filterPopupTextStyle.merge(dataGridThemeData.filterPopupTextStyle) : _filterPopupTextStyle;
    filterPopupDisabledTextStyle = dataGridThemeData.filterPopupDisabledTextStyle != null ? _filterPopupDisabledTextStyle.merge(dataGridThemeData.filterPopupDisabledTextStyle) : _filterPopupDisabledTextStyle;
    columnDragIndicatorColor = dataGridThemeData.columnDragIndicatorColor ?? colorScheme.primary;
    columnDragIndicatorStrokeWidth = dataGridThemeData.columnDragIndicatorStrokeWidth ?? 2;
  }
